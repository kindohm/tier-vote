    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      snap.forEach((d) => {
        ...
        data.push({
          ...
          createdAt: doc.createdAt?.toMillis ? doc.createdAt.toMillis() : doc.createdAt || Date.now(),
          text: doc.text,
...
export async function sendMessageToList(listId: string, user: { uid: string; displayName?: string | null; photoURL?: string | null } | null, text: string, clientId?: string) {
