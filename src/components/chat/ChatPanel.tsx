...
export const ChatPanel = ({ listId }: { listId?: string }) => {
  ...
  useEffect(() => {
    ...
    const serverClientIds = new Set(messages.map((m: any) => m.clientId).filter((c: any) => !!c));
    setOptimisticMessages((prev) => prev.filter((opt) => !opt.clientId || !serverClientIds.has(opt.clientId)));
  }, [messages]);
  ...
  const mergedMessages = (() => {
    ...
    const optimisticFiltered = optimisticMessages.filter((opt) => {
      ...
      if (opt.clientId && server.some((s) => s.clientId && opt.clientId && s.clientId === opt.clientId)) return false;
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setOptimisticMessages((prev) => prev.map((pm) => (pm.clientId === clientId ? { ...pm, failed: true } : pm)));
  ...
  return (
    ...
    >
      <button
        ...
        style={{ width: '40px', height: '40px', position: 'absolute', left: -40, top: 8 }}
        title={isExpanded ? "Collapse chat" : "Expand chat"}
        aria-label={isExpanded ? "Collapse chat" : "Expand chat"}
      >
        <span className={`bi bi-chevron-${isExpanded ? 'right' : 'left'}`} aria-hidden="true"></span>
        <span className="visually-hidden">{isExpanded ? 'Collapse chat' : 'Expand chat'}</span>
      </button>
              ...
              mergedMessages.map((m) => (
                <div key={m.id} className="d-flex mb-2">
                  <div className="me-2" style={{ width: 36 }}>
                    {m.photoURL ? (
                      <img src={m.photoURL} alt={m.userName || m.userId} className="rounded-circle" style={{ width: 36, height: 36, objectFit: 'cover' }} />
                    ) : (
                      <div className="rounded-circle bg-secondary text-white d-flex align-items-center justify-content-center" style={{ width: 36, height: 36, fontSize: 12 }}>
                  ...
                  <div style={{ flex: '1 1 auto' }}>
                    <div className="small text-muted mb-1">{m.userName || m.userId} · {new Date(m.createdAt).toLocaleTimeString()}</div>
                    <div className={`small bg-white border rounded px-2 py-1 ${ (m as any).failed ? 'border-danger bg-danger text-white' : ''}`}>
                      {m.text.split('\n').map((line, idx) => (
                        <span key={idx}>
                          {line}
                          {idx < m.text.split('\n').length - 1 ? <br /> : null}
